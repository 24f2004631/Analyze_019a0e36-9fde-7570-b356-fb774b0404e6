...
def main():
    ...
    daily_rev = (
        df.groupby(["region", "date"])["revenue"]  # daily revenue per region - FIXED TYPO HERE
        .sum()
        .reset_index()
    ...
    # The rolling window needs to be on a datetime index.
    # `g.set_index("date")["revenue"].rolling("7D").mean()` is the correct way.
    rolling = (
